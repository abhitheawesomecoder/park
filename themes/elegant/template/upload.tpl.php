	  ...
	  $name = $name.".".$ext;
	  	if($ext == "jpg" or $ext == "jpeg" or $ext == "png"){
			...
			$name_media_gif = $name.".jpg";
				...
				$upload = uploadImageFromURL2($url, $name_gif);
if($upload) {
	  			if($title && $media_type && $name && $category){
					...
					if($media_type == 'gif') {
						create_thumb($document."/uploads/media_photos/".$name_media_gif, $document."/uploads/media_photos/"."thumb_".$name_media_gif, 630);
					} elseif($media_type == 'pic') {
						create_thumb($document."/uploads/media_photos/".$name, $document."/uploads/media_photos/"."thumb_".$name, 630);
					}
...
?>
                        ...
                        <div class="field category">
                        	<select name="category" style="height: 36px;width: 560px;border-radius: 3px;border: 1px solid #ddd;">
                        		...
                        		<option value="<?=$option_cat['id']?>" <? if($_POST['category'] == $option_cat['id']) {?>selected<? }?>><? if($_SESSION['lang'] == 'de') { echo $option_cat['name']; } elseif($_SESSION['lang'] == 'en') { echo $option_cat['name_en']; } elseif($_SESSION['lang'] == 'th') { echo $option_cat['name_th']; } ?></option>
                                <? endforeach; ?>
                        	</select>
                        </div>
	  ...
	  $description = mysql_real_escape_string($_POST['img_description']);
		...
		$media_type = 'pic';
	  	if($title && $category){
			...
			create_thumb($document."/uploads/media_photos/".$mouse_multiple['file_name'], $document."/uploads/media_photos/"."thumb_".$mouse_multiple['file_name'], 630);
...
?>
                        ...
                        <p class="Lead_Title" style="color:#999"><?=$LANG['Upload_funny_pictures2'];?></p>
                        ...
                        <div class="field category">
                        	<select name="img_category" style="height: 36px;width: 560px;border-radius: 3px;border: 1px solid #ddd;">
                                ...
                                <? $option_cat = get_option_cat(); 
								   foreach($option_cat as $option_cat): ?>
                        		<option value="<?=$option_cat['id']?>" <? if($_POST['img_category'] == $option_cat['id']) {?>selected<? }?>><? if($_SESSION['lang'] == 'de') { echo $option_cat['name']; } elseif($_SESSION['lang'] == 'en') { echo $option_cat['name_en']; } elseif($_SESSION['lang'] == 'th') { echo $option_cat['name_th']; } ?></option>
                                <? endforeach; ?>
                        	</select>
                        </div>
...
?>
                        ...
                        <div class="field category">
                        	<select name="video_category" style="height: 36px;width: 560px;border-radius: 3px;border: 1px solid #ddd;">
                                ...
                                <? $option_cat = get_option_cat(); 
								   foreach($option_cat as $option_cat): ?>
                        		<option value="<?=$option_cat['id']?>" <? if($_POST['video_category'] == $option_cat['id']) {?>selected<? }?>><? if($_SESSION['lang'] == 'de') { echo $option_cat['name']; } elseif($_SESSION['lang'] == 'en') { echo $option_cat['name_en']; } elseif($_SESSION['lang'] == 'th') { echo $option_cat['name_th']; } ?></option>
                                <? endforeach; ?>
                        	</select>
                        </div>
