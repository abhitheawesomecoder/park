...
?>
<div class="box">
<a href="" style="color: #000; font-size: 13px; line-height: 24px;">
  <font style="font-weight: bold;padding: 3px 0;border-bottom: 1px solid #000;">
    <?=$first_name . " " . $last_name;?></font> &nbsp;
<font style="letter-spacing: 0.3px;color:#ee0005;">
  <? if($media['activity'] == 'comment') {
      if($media['activity_sub'] == 'like') {
        ?><?=$LANG['upvoted_and'];?>
        <? }?><?=$LANG['commented'];?>
        <? } if($media['activity_sub'] == 'like') {
          if($media['activity'] == 'comment') {
...
$cat_query = mysql_query("SELECT * FROM categories WHERE id=".$media['cat']."");
$cat_row = mysql_fetch_row($cat_query);
if($cat_row['2'] == '') {
  $cat_row = 'other';
}else{
    $cat_row=$cat_row['2'];
  }
  if($SETTINGS['permalink'] == 'gag') {
    echo "/gag/".$media['news_id'];
  } elseif($SETTINGS['permalink'] == 'cat') {
    echo "/".$cat_row."/".$media['news_id'];
  } elseif($SETTINGS['permalink'] == 'cat_slugify') {
    echo "/".$cat_row."/".slugify($media['title']);
  } ?>">
  <div class="box-media-title"><?=$media['title']?></div>
<?
$cat_display = mysql_query("SELECT * FROM categories WHERE id=".$media['cat']."");
$cat_display_row = mysql_fetch_row($cat_display);
?>
<?
if($media['type'] == 'pic') {
  echo '<div class="gif-post"><img src='.$root."/uploads/media_photos/thumb_".$media['file'].' width="'.$box_type.'" alt="'.$media['title'].'" title="'.$media['title'].'"></a></div>';
}
if($media['type'] == 'gif') {
  echo '</a><div class="gif-post"><img class="gif-image animation" id='.$media['news_id'].' src='.$root."/uploads/media_photos/".$media['thumb'].' data-animation='.$root."/uploads/media_photos/".$media['file'].' data-original='.$root."/uploads/media_photos/".$media['thumb'].' data-state="0" width="'.$box_type.'" /> <span class="play">GIF</span></div>';
}
if($media['type'] == 'vid') {
  echo '<div class="gif-post"><img src='.$root."/uploads/media_photos/".$media['file'].' width="'.$box_type.'"><span class="play_video">VIDEO</span></a></div>';
}
?>
...
<? if($_GET['subaction'] == 'posts') { ?> <!-- POSTS -->
	...
	<div id="container" style="margin-left: 3px; margin-top: 5px;">
		...
		<? $i = 7;
		   ...
		   $query_user = mysql_query("SELECT * FROM users WHERE id='".$media['author']."'");
			 while($user_media = mysql_fetch_array($query_user)) {
				 $first_name = $user_media['first_name'];
				 $last_name = $user_media['last_name'];
			 }
		?>

		<div class="box">
		<a href="<?=$root;?><? $cat_query = mysql_query("SELECT * FROM categories WHERE id=".$media['cat']."");
		$cat_row = mysql_fetch_row($cat_query);
		if($cat_row['2'] == '') {
			$cat_row = 'other';
		}else{
...
<? $i = 7;
   ...
   $query_user = mysql_query("SELECT * FROM users WHERE id='".$media['user_id']."'");
	 while($user_media = mysql_fetch_array($query_user)) {
		 $first_name = $user_media['first_name'];
		 $last_name = $user_media['last_name'];
	 }
...
<div class="box">
<a href="" style="color: #000; font-size: 13px; line-height: 24px;">
	<font style="font-weight: bold;padding: 3px 0;border-bottom: 1px solid #000;"><?=$first_name . " " . $last_name;?></font> &nbsp;
	<font style="letter-spacing: 1px;color:#ee0005;"><?=$LANG['upvoted'];?></font>
</a>
...
<? if($_GET['subaction'] == 'comments') { ?> <!-- COMMENTS -->
	...
	<? $i = 7;
	   ...
	   $query_user = mysql_query("SELECT * FROM users WHERE id='".$media['member_id']."'");
		 while($user_media = mysql_fetch_array($query_user)) {
			 $first_name = $user_media['first_name'];
			 $last_name = $user_media['last_name'];
		 }
  ?>
	...
	<a href="" style="color: #000; font-size: 13px; line-height: 24px;">
		<font style="font-weight: bold;padding: 3px 0;border-bottom: 1px solid #000;"><?=$first_name . " " . $last_name;?></font> &nbsp;
		<font style="letter-spacing: 1px;color:#ee0005;"><?=$LANG['commented'];?></font>
	</a>
	<a href="<?=$root;?><? $cat_query = mysql_query("SELECT * FROM categories WHERE id=".$media['cat']."");
	$cat_row = mysql_fetch_row($cat_query);
	if($cat_row['2'] == '') {
